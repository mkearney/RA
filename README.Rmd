...
A repository for organizing my ideas and materials for research assistants
...
+ Compile lists of Twitter screen names (professional and personal) for the following:
...
### Data
+ Link to Google spreadsheet: https://docs.google.com/spreadsheets/d/1mvGdz2rONx1ToH_Kc8Cq6GA1MFtljL7ZZawV5TR8CqU/edit?usp=sharing
+ Data fields
   - `full_name` Name of organization/program/person
   - `screen_name` Screen name of organization/program/person
   - `personal` Logical, indicating whether the account is a "personal" (non-official) account
   - `type` Category describing the type of account. Must be one of following:
      + `politician`
      + `reporting`
      + `pundit`
      + `lobbying`
   - `subtype` Category describing the sub-type of account. Must be one of following:
   ...
   - `cabinet_position` Category describing the cabinet position if applicable. Must be one of following:
      + `President`
      + `Vice President`
   ...
   - `state` Senator/Representative/Governor state e.g., Missouri, Hawaii, Oregon, Virginia, etc.
   - `parent_org` Name of parent organization (if obvious) e.g., the parent of Fox News Politics is Fox news.
   - `medium` Category describing the type of media e.g., TV, radio, internet
